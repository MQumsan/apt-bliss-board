...
import { ViewUnitDialog, EditUnitDialog, PaymentDialog } from './UnitDialogs';
    ...
    <>
      <div className={cn(
        ...
        <div className="flex items-start justify-between mt-1">
          ...
          <span className={cn('inline-flex items-center gap-1.5 rounded-full px-2.5 py-0.5 text-xs font-medium', config.bg, config.fg)}>
            {statusLabel}
          </span>
        </div>
        ...
        <div className="mt-3 flex items-center gap-1 border-t border-border pt-3">
          <button onClick={() => setViewOpen(true)} className="flex-1 flex items-center justify-center gap-1 rounded-md py-1.5 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors" title={t('view')}>
            <Eye className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">{t('view')}</span>
          </button>
          <button onClick={() => setEditOpen(true)} className="flex-1 flex items-center justify-center gap-1 rounded-md py-1.5 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors" title={t('edit')}>
            <Pencil className="h-3.5 w-3.5" />
            <span className="hidden sm:inline">{t('edit')}</span>
          </button>
          <button onClick={() => setPaymentOpen(true)} className="flex-1 flex items-center justify-center gap-1 rounded-md py-1.5 text-xs text-muted-foreground hover:bg-accent hover:text-accent-foreground transition-colors" title={t('payment')}>
      ...
      <PaymentDialog unit={unit} open={paymentOpen} onOpenChange={setPaymentOpen} />
    </>
  );
}
